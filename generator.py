...
def _fix_double_typing(content: str) -> str:
    ...
    대상:
      - 태그 중복: [해해석] → "", [전전망] → "" (오형성 태그 즉시 제거)
      - 공백 포함 태그: [해 해석] → "", [전 전망] → ""
      - 한글 2~4자 단어가 공백 하나를 사이에 두고 즉시 반복: "미 미", "을 을" 등
      - 연속 공백 → 단일 공백
    """
    # 1. 태그 중복 제거 (오형성 태그 즉시 삭제)
    content = re.sub(r"\[해해석\]",  "", content)
    content = re.sub(r"\[전전망\]",  "", content)
    content = re.sub(r"\[해\s해석\]", "", content)
    content = re.sub(r"\[전\s전망\]", "", content)
...
- [날짜불명] 마커가 붙은 자료는 핵심 근거로 절대 사용 금지. 언급 시 "출처·시점 불명" 명시 필수.
- 동일 기사·리서치를 서로 다른 섹션에서 중복 인용 금지. 각 출처는 1회만 근거로 사용한다.
...
8. 기계적인 접속사 패턴 반복 금지
9. 출처 다양성 하한: 참고 자료 섹션에 서로 다른 기관·매체 출처가 최소 3개 이상 포함되어야 한다.
   - 동일 출처(기관명)의 복수 항목은 1개 출처로 간주한다.
   - 3개 미만이면 추가 출처를 명시하거나 해당 수치 근거를 보완한다.
...
□ ★ [출처 다양성] 참고 자료 섹션에 서로 다른 기관·매체 출처가 최소 3개 이상 포함됐는가 — 동일 기관 복수 항목은 1개로 간주하며, 3개 미만이면 즉시 추가할 것
...
- 출처 다양성 하한: 참고 자료 섹션에 서로 다른 기관·매체 출처가 최소 2개 이상 포함되어야 한다.
  동일 기관의 복수 리포트는 1개 출처로 간주한다. 2개 미만이면 업황 데이터 또는 뉴스 출처로 보완한다.
...
□ [Phase 21 SEO_TITLE 검수] 출력 첫 줄이 <!-- SEO_TITLE: ... --> 형식 주석으로 시작하는가
□ ★ [출처 다양성] 참고 자료 섹션에 서로 다른 기관·매체 출처가 최소 2개 이상 포함됐는가 — 동일 기관 복수 리포트는 1개로 간주하며, 2개 미만이면 즉시 추가할 것
...
[Phase 15C/16 — 미래 연도 FORECAST 문장 [전망] 브래킷 태그 삽입 금지]
FORECAST 상태(현재 연도 미래 월 및 다음 연도 이후)에 해당하는 전망·예측 문장에 대해서도:
  ❌ [전망] 브래킷 태그를 원문에 없는 위치에 새로 삽입하는 것은 금지됩니다
  ✅ 조건부 표지가 필요하면 동사 어미('~로 예상된다', '~할 것으로 전망된다')를 문장 내에 추가하십시오
  ✅ 원문에 이미 있는 [전망] 태그는 그대로 보존하십시오
